...
export class Transfer implements CompatEntity {
    ...
    static async getByBlockNumber(blockNumber: number, options: GetOptions<CompatTransferProps>): Promise<Transfer[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatTransferProps>('Transfer', 'blockNumber', blockNumber, options);
        return records.map(record => this.create(record as unknown as TransferProps));
    }
    

    static async getByExtrinsicId(extrinsicId: number, options: GetOptions<CompatTransferProps>): Promise<Transfer[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatTransferProps>('Transfer', 'extrinsicId', extrinsicId, options);
        return records.map(record => this.create(record as unknown as TransferProps));
    }
    

    static async getByTimestamp(timestamp: Date, options: GetOptions<CompatTransferProps>): Promise<Transfer[]> {
        // Inputs must be cast as the store interface has not been updated to support alternative ID types
        const records = await store.getByField<CompatTransferProps>('Transfer', 'timestamp', timestamp, options);
        return records.map(record => this.create(record as unknown as TransferProps));
    }
