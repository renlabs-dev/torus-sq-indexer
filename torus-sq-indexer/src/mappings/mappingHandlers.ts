...
export async function handleTransfer(event: SubstrateEvent): Promise<void> {
  ...
  try {
    ...
    // Update sender's free balance
    const fromAccount = await Account.get(from);
    if (fromAccount) {
      fromAccount.balance_free -= amount;
      fromAccount.balance_total = fromAccount.balance_free + fromAccount.balance_staked;
      fromAccount.updatedAt = blockNumber;
      await fromAccount.save();
    }

    // Update receiver's free balance (create account if new)
    ...
    if (!toAccount) {
      ...
      toAccount.balance_free += amount;
      toAccount.balance_total = toAccount.balance_free + toAccount.balance_staked;
